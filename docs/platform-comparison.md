...
| **Discussions** | 質疑応答、相談、雑談 | ⚠️ パブリックのみ |
| **Issues** | コンテンツの問題報告、改善提案 | ✅ |
| **Projects** | 進捗の可視化（オプション） | ✅ |

> ⚠️ **注意**: GitHub Discussionsは**プライベートリポジトリでは有料（Team以上）**が必要です。
> プライベートの場合はSlackで代替してください。
...
**使い分け**:
- **技術的な質問・ナレッジ共有** → GitHub Discussions（検索・蓄積される）
- **気軽な相談・雑談・緊急連絡** → Slack（リアルタイム性）
